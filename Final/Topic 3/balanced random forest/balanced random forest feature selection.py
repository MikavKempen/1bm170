...
# Use a temporary RF model to get top features
temp_rf = RandomForestClassifier(n_estimators=100, random_state=42)
temp_rf.fit(X_train_val, y_train_val)
...
# === Final train/val split after feature selection ===
...
# === Final Feature Importance from selected features ===
final_feat_imp = pd.Series(clf.feature_importances_, index=top_features).sort_values(ascending=False)

plt.figure(figsize=(12, max(6, len(final_feat_imp) * 0.3)))
sns.barplot(x=final_feat_imp.values, y=final_feat_imp.index)
