...
# Basic RF model to get importances
...
print("Reduced shape (RF):", X_selected_rf.shape)

# Get selected feature names
selected_features_rf = X.columns[selector_rf.get_support()]
print("Selected features by RF:", selected_features_rf.tolist())
...
for maj_ratio, min_ratio in ratios:
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X_selected_rf, y), 1):
        X_train, X_test = X_selected_rf[train_idx], X_selected_rf[test_idx]
        ...
        plot_feature_importance(model_sm, selected_features_rf, f"SMOTE Features - Fold {fold} - Ratio {int(maj_ratio*100)}:{int(min_ratio*100)}")
        ...
        plot_feature_importance(model_us, selected_features_rf, f"Undersample Features - Fold {fold} - Ratio {int(maj_ratio*100)}:{int(min_ratio*100)}")
...
for fold, (train_idx, test_idx) in enumerate(skf.split(X_selected_rf, y), 1):
    ...
    plot_feature_importance(model_us_weighted, selected_features_rf, f"Undersample + Weights - Features - Fold {fold}")
...
for fold, (train_idx, test_idx) in enumerate(skf.split(X_selected_rf, y), 1):
    ...
    plot_feature_importance(model_xgb, selected_features_rf, f"XGBoost Features - Fold {fold}")
...
for fold, (train_idx, test_idx) in enumerate(skf.split(X_selected_rf, y), 1):
    ...
    plot_feature_importance(model_xgb, selected_features_rf, f"XGBoost Tuned Features - Fold {fold}")
